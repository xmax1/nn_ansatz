...
def compute_gr(rs, distances, volume, n_points):
    ...
    n_walkers, n_ri, n_rj = distances.shape  # number of reference particles, number target
    number_density = n_rj / volume  # is the number density n_ri or 
    
    volume_element = (4.*jnp.pi/3.) * (rs[1:]**3 - rs[:-1]**3)
    hist, _ = jnp.histogramdd(distances.reshape(-1, 1), bins=(n_points-1), range=[(rs[0], rs[-1]),]) 
    pdf = hist / (n_walkers * volume_element * number_density * n_ri)
...
def compute_pr(walkers: jnp.ndarray, 
    ...
    walkers_r = keep_in_boundary(walkers_r, basis, inv_basis)
...
def compute(run_dir: str='./experiments/HEG_PRX/bf_af_0/BfCs/seed0/run_0',
    ...
    rs = jnp.linspace(0.0, max_distance, n_points)[1:]
    t0 = time.time()
    for i, walkers in enumerate(all_walkers):
        ...
        if i % 10 == 0:
            t1 = time.time()
            print(f'Proportion complete: {(i*n_walkers_max)/float(n_walkers_all)} time {(t1 - t0):.0f}')
            t0 = t1
    ...
    save_pk(d, oj(run_dir, f'exp_stats_pair_corr_d{[1, 3][d3]}_new.pk'))
